...
SQLConnector.prototype._buildWhere = function(model, where) {
  ...
  for (const key in where) {
    ...
    if (key === 'fts') {
      ...
      const key0 = Array.isArray(matchObj) ? matchObj[0] : matchObj
      const definition =  props[key0];
      const against = self.toColumnValue(definition , where[key]["against"]);
      stmt.merge({
        sql: `MATCH(${match}) AGAINST(? IN BOOLEAN MODE)`,
        params: [against],
      });
      whereStmts.push(stmt);
      continue;
